        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (!isPrems && plugin.diamond && global.db.data.users[m.sender].diamond < plugin.diamond * 1) {
                    this.reply(m.chat, `*ⓘ* Tus *Yuans/Yens* se agotaron\nuse el siguiente comando para comprar más *Yens*\n\n*${usedPrefix}buy*`, m)
                ...
                try {
                    ...
                    if (m.diamond)
                        m.reply(`Utilizaste *${+m.diamond}* ◜💴◞ Yens/Yuans`)
                }
                break
            }
        }
...
global.dfail = (type, m, conn) => {
    ...
    if (msg) return conn.reply(m.chat, msg, m, { contextInfo:{ forwardingScore: 2022, isForwarded: true, externalAdReply: {title: '𝗦𝘂𝗺𝗶𝗪𝗮𝗕𝗼𝘁 ┃𝗜𝗻𝗳𝗼𝗿𝗺𝗮𝗰𝗶𝗼𝗻', body: ucapan(), sourceUrl: global.fgyt, thumbnail: imagen2 }}})
}
...
function ucapan() {
  ...
  if (time >= 4) {
    res = "Buenos Días Querido Usuario 🌅"
  }
  if (time >= 10) {
    res = "Buenas Tardes Querido Usuario ☀️"
  }
  if (time >= 15) {
    res = "Buenas Noches Querido Usuario 🌌"
  }
  if (time >= 18) {
    res = "Buenas Madrugadas Querido Usuario 🌄"
  }
  return res
}
